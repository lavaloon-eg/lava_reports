...
def get_result(filters, account_details):
    ...
    if filters.get("show_unposted_transactions"):
        result[1:1] = get_unposted_gl_data(filters)
    return result


def get_unposted_gl_data(filters):
    unposted_data: list = []
    unposted_data = get_doctype_data_preview(unposted_data=unposted_data,
                                             doctype="Purchase Invoice", filters=filters)
    unposted_data = get_doctype_data_preview(unposted_data=unposted_data,
                                             doctype="Payment Entry", filters=filters)
    unposted_data = get_doctype_data_preview(unposted_data=unposted_data,
                                             doctype="Journal Entry", filters=filters)

    return map_to_dict(unposted_data)


def get_unposted_doctype_list(doctype, filters):
    cost_center_cond = ""
    project_cond = ""
    account_cond = ""
    party_cond = ""
    ...
    return frappe.db.sql(
        f"""
            ...
            WHERE
                docstatus = 0
                AND company = '{filters.get("company")}'
...
def get_doctype_data_preview(unposted_data, doctype, filters):
    doc_id_list = get_unposted_doctype_list('tab' + doctype, filters)
    for doc_id in doc_id_list:
        data_preview = show_accounting_ledger_preview(
            company="Lavaloon", doctype=doctype, docname=doc_id).get("gl_data")
        if data_preview:
            for it in data_preview:
                it[0] = "Unposted Date"
                it.append(doctype)
                it.append(doc_id[0])
            unposted_data = unposted_data + data_preview
    return unposted_data


def map_to_dict(unposted_data):
    result = []
    data_map_list = ["posting_date", "account", "debit", "credit", "against", "party_type", "party",
                     "cost_center", "against_voucher_type", "against_voucher", "voucher_type", "voucher_no"]
    for entry in unposted_data:
        entry_dict = {"gl_entry": "Not Posted"}
        for i in range(len(data_map_list)):
            entry_dict[data_map_list[i]] = entry[i]
